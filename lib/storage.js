...
export async function initDb() {
  ...
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS test_plan_presets (
      id TEXT PRIMARY KEY,
      data JSONB NOT NULL
    );
  `);
...
export const testPlanPresets = {
  async getAll({ productType } = {}) {
    if (pool) {
      let q = 'SELECT data FROM test_plan_presets';
      const vals = [];
      if (productType) { q += ` WHERE data->>'productType' = $1`; vals.push(productType); }
      q += " ORDER BY (data->>'name')";
      const { rows } = await pool.query(q, vals);
      return rows.map(r => r.data);
    }
    const file = join(filePaths().DATA_BASE, 'test-plan-presets.json');
    let all = await readJson(file);
    if (productType) all = all.filter(p => p.productType === productType);
    return all;
  },

  async getById(id) {
    if (pool) {
      const { rows } = await pool.query('SELECT data FROM test_plan_presets WHERE id = $1', [id]);
      return rows[0]?.data || null;
    }
    const all = await readJson(join(filePaths().DATA_BASE, 'test-plan-presets.json'));
    return all.find(e => e.id === id) || null;
  },

  async create(entry) {
    if (pool) {
      await pool.query('INSERT INTO test_plan_presets (id, data) VALUES ($1, $2)', [entry.id, entry]);
      return entry;
    }
    const file = join(filePaths().DATA_BASE, 'test-plan-presets.json');
    const all = await readJson(file);
  ...
  async update(id, patch) {
    if (pool) {
      ...
      if (!rows[0]) return null;
      const updated = { ...rows[0].data, ...patch, id };
      await pool.query('UPDATE test_plan_presets SET data = $1 WHERE id = $2', [updated, id]);
      return updated;
    }
    const file = join(filePaths().DATA_BASE, 'test-plan-presets.json');
    const all = await readJson(file);
  ...
  async delete(id) {
    if (pool) {
      await pool.query('DELETE FROM test_plan_presets WHERE id = $1', [id]);
      return;
    }
    const file = join(filePaths().DATA_BASE, 'test-plan-presets.json');
    const all = await readJson(file);
